...
import { runtimeState, summarizeNumericField, summarizeStringField, buildTimeline, TIMELINE_MAX, } from '../../tools/runtime-state.js';
...
describe('runtimeState tool', () => {
    ...
    describe('schema validation', () => {
        ...
        it('accepts digest with all params', () => {
            expect(runtimeState.schema.safeParse({
                action: 'digest',
                select: 'group',
                group: 'my_watch',
        ...
        it('accepts select="none" with explicit paths', () => {
            expect(runtimeState.schema.safeParse({ action: 'digest', select: 'none', paths: ['/root/GameState'] }).success).toBe(true);
        });
        ...
        it('rejects watch_start with neither specs nor signals (incl. empty arrays)', () => {
            ...
            expect(runtimeState.schema.safeParse({ action: 'watch_start', specs: [], signals: [] }).success).toBe(false);
        });
        ...
        it('rejects a signal entry missing the signal name', () => {
            expect(runtimeState.schema.safeParse({ action: 'watch_start', signals: [{ path: '/root/G' }] }).success).toBe(false);
        });
        it('rejects blank signal paths and names (blank entries would skew the drop detection)', () => {
            expect(runtimeState.schema.safeParse({ action: 'watch_start', signals: [{ path: '', signal: 'died' }] }).success).toBe(false);
            expect(runtimeState.schema.safeParse({ action: 'watch_start', signals: [{ path: '/root/G', signal: '' }] }).success).toBe(false);
        });
    });
    // ── digest ───────────────────────────────────────────────────────────────
    describe('digest', () => {
        ...
        it('passes through state snapshot, available_autoloads, and unresolved_paths', async () => {
            ...
            mock.mockResponse(response);
            const ctx = createToolContext(mock);
            const result = await runtimeState.execute({ action: 'digest', select: 'none', paths: ['/root/GameState', '/root/Nope'] }, ctx);
    ...
    describe('watch_start', () => {
        ...
        it('uses defaults for hz and duration_ms when omitted', async () => {
            ...
            const ctx = createToolContext(mock);
            await runtimeState.execute({ action: 'watch_start', specs: [{ path: '/root/Player', fields: ['pos.x'] }] }, ctx);
        ...
        it('passes signals through and defaults them to [] when omitted', async () => {
            ...
            await runtimeState.execute({ action: 'watch_start', signals: [{ path: '/root/G', signal: 'wave_started' }] }, ctx);
        ...
        it('passes unresolved signals through structurally and names them in warnings', async () => {
            ...
            const ctx = createToolContext(mock);
            const result = await runtimeState.execute({ action: 'watch_start', signals: [{ path: '/root/Bogus', signal: 'died' }] }, ctx);
            const data = structuredOf(result);
            expect(data.unresolved_signals).toEqual([{ path: '/root/Bogus', signal: 'died', reason: 'node_not_found' }]);
            expect(data.warnings.some((w) => w.includes('/root/Bogus:died (node_not_found)'))).toBe(true);
        });
        it('does not warn about 0 fields on a signals-only watch', async () => {
            ...
            const ctx = createToolContext(mock);
            const result = await runtimeState.execute({ action: 'watch_start', signals: [{ path: '/root/G', signal: 'wave_started' }] }, ctx);
        ...
        it('still warns when specs were requested but none resolved', async () => {
            ...
            const ctx = createToolContext(mock);
            const result = await runtimeState.execute({ action: 'watch_start', specs: [{ path: '/root/Nope', fields: ['pos.x'] }] }, ctx);
            expect(structuredOf(result).warnings.some((w) => w.includes('0 fields resolved'))).toBe(true);
        });
        it('detects an addon that predates the timeline (signals silently ignored)', async () => {
            ...
            const data = structuredOf(result);
            expect(data.warnings.some((w) => w.includes('update the godot-mcp addon'))).toBe(true);
        });
        it('detects a stale editor relay dropping signal specs in transit', async () => {
            ...
            const data = structuredOf(result);
            expect(data.warnings.some((w) => w.includes('restart the Godot editor'))).toBe(true);
        });
        it('raises no skew warning when every requested signal is accounted for', async () => {
            ...
            const data = structuredOf(result);
            expect(data.warnings.some((w) => w.includes('restart') || w.includes('update'))).toBe(false);
        });
    });
    // ── watch_collect ────────────────────────────────────────────────────────
    describe('watch_start', () => {
        it('surfaces unresolved_fields with a warning', async () => {
            ...
            const ctx = createToolContext(mock);
            const result = await runtimeState.execute({ action: 'watch_start', specs: [{ path: '/root/HUD/Bar', fields: ['pos.y', 'x'] }] }, ctx);
            const data = structuredOf(result);
        ...
        it('degrades to an empty unresolved_fields list against an older addon', async () => {
            mock.mockResponse({ started: true, resolved_fields: 2 });
            const ctx = createToolContext(mock);
            const result = await runtimeState.execute({ action: 'watch_start', specs: [{ path: '/root/P', fields: ['pos.x'] }] }, ctx);
            const data = structuredOf(result);
    ...
    describe('watch_collect', () => {
        ...
        it('merges signal events and string transitions into a time-sorted timeline', async () => {
            ...
            const result = await runtimeState.execute({ action: 'watch_collect' }, ctx);
            const data = structuredOf(result);
            expect(data.timeline.map((e) => [e.t_ms, e.kind])).toEqual([
                [120, 'signal'],
                [400, 'anim_transition'],
                [880, 'signal'],
                [900, 'field_change'],
        ...
        it('caps the merged timeline at TIMELINE_MAX and flags it', async () => {
            ...
            const samples = [];
            for (let i = 0; i < 600; i++)
                samples.push({ t_ms: i, value: i % 2 === 0 ? 'a' : 'b' });
    ...
    it('ignores numeric field summaries entirely', () => {
        const fields = {
            '/root/P:vel.x': {
                ...
                events: [{ t_ms: 50, from: 0, to: 5, kind: 'zero_cross' }],
            },
        };
