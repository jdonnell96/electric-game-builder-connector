...
describe('godot_docs tool', () => {
    ...
    describe('mocked (no network)', () => {
        ...
        it('builds a lowercased class URL and returns full markdown for section=full', async () => {
            ...
            const result = (await docs.execute({ action: 'fetch_class', class_name: 'Node2D', version: 'stable', section: 'full' }, ctx));
            expect(spy).toHaveBeenCalledOnce();
            expect(spy.mock.calls[0][0]).toBe('https://docs.godotengine.org/en/stable/classes/class_node2d.html');
        ...
        it('section=description returns the title + Description only', async () => {
            ...
            const ctx = createToolContext(createMockGodot());
            const result = (await docs.execute({ action: 'fetch_class', class_name: 'Node2D', version: 'stable', section: 'description' }, ctx));
            expect(result).toContain('# Node2D');
            expect(result).toContain('## Description');
        ...
        it('section=methods returns just the Methods section', async () => {
            mockFetch(CLASS_HTML);
            const ctx = createToolContext(createMockGodot());
            const result = (await docs.execute({ action: 'fetch_class', class_name: 'Node2D', version: 'stable', section: 'methods' }, ctx));
            expect(result).toContain('## Methods');
        ...
        it('reports a missing named section instead of throwing', async () => {
            ...
            const ctx = createToolContext(createMockGodot());
            const result = (await docs.execute({ action: 'fetch_class', class_name: 'Empty', version: 'stable', section: 'signals' }, ctx));
        ...
        it('reports a missing Description section instead of throwing', async () => {
            ...
            const ctx = createToolContext(createMockGodot());
            const result = (await docs.execute({ action: 'fetch_class', class_name: 'NoDesc', version: 'stable', section: 'description' }, ctx));
        ...
        it('fetch_page normalizes a path without a leading slash', async () => {
            ...
            const ctx = createToolContext(createMockGodot());
            const result = (await docs.execute({ action: 'fetch_page', path: 'tutorials/2d/2d_movement.html', version: 'stable' }, ctx));
            expect(spy.mock.calls[0][0]).toBe('https://docs.godotengine.org/en/stable/tutorials/2d/2d_movement.html');
            expect(result).toContain('Source: https://docs.godotengine.org');
        });
        it('auto-detects the docs version from the connected Godot', async () => {
            ...
            expect(spy.mock.calls[0][0]).toContain('/en/4.5/');
        ...
        it('falls back to stable when no Godot version is available', async () => {
            ...
            const result = (await docs.execute({ action: 'fetch_class', class_name: 'Node2D', section: 'description' }, ctx));
            expect(spy.mock.calls[0][0]).toContain('/en/stable/');
        ...
        it('an explicit version overrides auto-detection and omits the note', async () => {
            ...
            expect(spy.mock.calls[0][0]).toContain('/en/4.4/');
            expect(result).not.toContain('auto-detected');
        });
        it('maps a 404 to a not-found error', async () => {
            mockFetch('', 404);
            const ctx = createToolContext(createMockGodot());
            await expect(docs.execute({ action: 'fetch_class', class_name: 'Ghost', version: 'stable', section: 'full' }, ctx)).rejects.toThrow('not found');
        });
        it('surfaces a non-OK HTTP status (e.g. rate limiting)', async () => {
            mockFetch('', 429);
            const ctx = createToolContext(createMockGodot());
            await expect(docs.execute({ action: 'fetch_page', path: '/x.html', version: 'stable' }, ctx)).rejects.toThrow('HTTP 429');
        });
        it('maps an aborted fetch to a timeout error', async () => {
            ...
            const ctx = createToolContext(createMockGodot());
            await expect(docs.execute({ action: 'fetch_class', class_name: 'Node2D', version: 'stable', section: 'full' }, ctx)).rejects.toThrow('timed out');
        });
        it('explains when the page has no extractable main content', async () => {
            mockFetch('<html><body>no main wrapper</body></html>');
            const ctx = createToolContext(createMockGodot());
            await expect(docs.execute({ action: 'fetch_class', class_name: 'Node2D', version: 'stable', section: 'full' }, ctx)).rejects.toThrow('structure may have changed');
        });
        it('requires a non-empty class_name', async () => {
            const ctx = createToolContext(createMockGodot());
            await expect(docs.execute({ action: 'fetch_class', class_name: '', version: 'stable', section: 'full' }, ctx)).rejects.toThrow('class_name is required');
        });
        it('requires a non-empty path for fetch_page', async () => {
            const ctx = createToolContext(createMockGodot());
            await expect(docs.execute({ action: 'fetch_page', path: '', version: 'stable' }, ctx)).rejects.toThrow('path is required');
        });
    });
