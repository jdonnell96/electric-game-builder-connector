...
describe('input tool', () => {
    ...
    describe('key entries (#290)', () => {
        describe('schema', () => {
            ...
            it('names the key shape in the union error for a no-discriminator entry', () => {
                ...
                if (!r.success)
                    expect(JSON.stringify(r.error.issues)).toContain('{key, physical?}');
            });
        });
    ...
    describe('look entries (#294)', () => {
        describe('schema', () => {
            ...
            it('names the look shape in the union error for a no-discriminator entry', () => {
                ...
                expect(r.success).toBe(false);
                if (!r.success)
                    expect(JSON.stringify(r.error.issues)).toContain('{look: [dx, dy]}');
            });
        });
    ...
    describe('JSON Schema draft 2020-12 compliance', () => {
        ...
        function findDraft07TupleViolations(node, path, out) {
            ...
            if (node && typeof node === 'object') {
                ...
                if (Array.isArray(obj.items))
                    out.push(`${path}.items is an array (draft-07 tuple form)`);
                if ('additionalItems' in obj)
                    out.push(`${path}.additionalItems present (draft-07 only)`);
                for (const [k, v] of Object.entries(obj))
                    findDraft07TupleViolations(v, `${path}.${k}`, out);
            }
        }
