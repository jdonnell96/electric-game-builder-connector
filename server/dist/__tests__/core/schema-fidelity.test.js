...
const flat = () => toInputSchema(TestUnion);
...
describe('flattened schemas preserve per-action information', () => {
    ...
    it('conflicting descriptions are kept per action instead of silently dropped', () => {
        const description = props().conflicted.description;
    ...
    it('real tools publish their per-action guidance', () => {
        const sceneProps = toInputSchema(scene.schema).properties;
        ...
        const rsProps = toInputSchema(runtimeState.schema).properties;
        // The watch lifecycle guidance must reach the model, not just the docs
        expect(rsProps.action.description.length).toBeGreaterThan(50);
    });
});
...
//# sourceMappingURL=schema-fidelity.test.js.map