...
const VERBOSE_LEVELS = new Set(['debug', 'info', 'notice']);
...
function log(level, message, data) {
    if (VERBOSE_LEVELS.has(level) && !isVerboseEnabled())
        return;
    const suffix = data ? ` ${JSON.stringify(data)}` : '';
    console.error(`[${LOGGER_NAME}] [${level}] ${message}${suffix}`);
}
function logRateLimited(level, key, message, data) {
...
export const logger = {
    ...
    warningRateLimited: (key, message, data) => logRateLimited('warning', key, message, data),
};
