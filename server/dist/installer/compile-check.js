...
import { findGodotCandidates } from './godot-locator.js';
import { formatScriptError, parseGodotLog } from './godot-log.js';
...
export async function runAddonCompileCheck(installedAddonDir, opts = {}) {
    ...
    if (opts.candidates)
        ...
        candidates = findGodotCandidates(opts.godot).filter((c) => c.source === '--godot');
    else
        candidates = findGodotCandidates();
    ...
    if (!godot) {
        ...
        return {
            status: 'skipped',
            summary: 'no Godot 4 executable found (looked at GODOT_BIN, GODOT_PATH, PATH, and the usual install folders). Pass --godot <path-to-godot> to run the check.',
            errors: [],
            otherErrors: [],
        };
    }
...
export function formatCompileCheck(result) {
    const lines = [];
    switch (result.status) {
        ...
        case 'failed':
            ...
            for (const err of result.errors)
                lines.push(formatScriptError(err));
            if (result.errors.length === 0) {
                for (const other of result.otherErrors.slice(0, 5))
                    lines.push(`  ${other}`);
            }
            if (result.logPath)
                lines.push(`  Full Godot output: ${result.logPath}`);
            break;
    }
    if (result.godot)
        lines.push(`  (${result.godot.path}, found via ${result.godot.source})`);
    return lines.join('\n');
}
/** The first candidate that answers --version as a Godot 4. */
export async function findWorkingGodot(candidates) {
    for (const candidate of candidates) {
        const answer = await queryVersion(candidate.path);
        if (answer && answer.version.startsWith('4.'))
            return { ...candidate, ...answer };
    }
    return undefined;
}
async function pickWorkingGodot(candidates) {
...
async function queryVersion(path) {
    try {
...
function runGodot(path, args, timeoutMs) {
...
function parseGodotVersionMin(pluginCfgPath) {
    try {
...
export function throwawayProjectFile(port) {
    return [
        'config_version=5',
        '',
        '[application]',
...
//# sourceMappingURL=compile-check.js.map