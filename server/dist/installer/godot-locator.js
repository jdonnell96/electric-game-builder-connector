...
export function findGodotCandidates(explicit, opts = {}) {
    ...
    const add = (path, source) => {
        ...
        if (!resolved || seen.has(resolved.toLowerCase()))
            return;
        if (!isExecutableFile(resolved, platform))
            return;
    ...
    for (const hit of whichAll(PATH_NAMES, env, platform))
        add(hit, 'PATH');
    for (const { path, source } of wellKnownLocations(platform, env, home))
        add(path, source);
    for (const { dir, source } of scanDirectories(platform, env, home)) {
        for (const found of scanForGodot(dir, platform, 2))
            add(found, source);
    }
    return out;
}
/** macOS: a `Godot.app` bundle stands in for the binary inside it. */
function resolveAppBundle(path, platform) {
...
function whichAll(names, env, platform) {
...
function wellKnownLocations(platform, env, home) {
    if (platform === 'win32') {
...
function scanDirectories(platform, env, home) {
    const dirs = [
...
export function scanForGodot(dir, platform, depth) {
    const found = [];
    const visit = (current, remaining) => {
        ...
        for (const entry of entries) {
            ...
            if (!entry.isFile() || !NAME_RE.test(entry.name))
                continue;
            if (platform === 'win32' && !/\.exe$/i.test(entry.name))
                continue;
            if (platform !== 'win32' && /\.(zip|sha256|txt|md|tpz|pck|exe|dmg)$/i.test(entry.name))
                continue;
...
//# sourceMappingURL=godot-locator.js.map