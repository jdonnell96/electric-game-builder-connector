...
export const JOY_AXIS_NAMES = ['left_x', 'left_y', 'right_x', 'right_y', 'trigger_left', 'trigger_right'];
...
const ActionEntrySchema = z.strictObject({
    ...
    strength: z
        ...
        .max(1)
        .optional()
        .describe('Analog press strength (default 1.0). Drives Input.get_action_strength/get_vector even for ' +
...
const KeyEntrySchema = z.strictObject({
    key: z
        ...
        .describe('Raw keyboard key by name with optional modifier prefixes ("a", "escape", "ctrl+s", ' +
    ...
    physical: z
        .boolean()
        .optional()
        .describe('Target the physical key position (layout-independent): sends a physical-keycode-only event ' +
...
const LookEntrySchema = z.strictObject({
    look: z
        ...
        .array(z.number())
        .length(2)
        .describe('Relative mouse-look: a mouse movement of [dx, dy] SCREEN pixels (+x = right, +y = down), ' +
...
export const InputEntrySchema = z.union([ActionEntrySchema, JoyButtonEntrySchema, AxisEntrySchema, StickEntrySchema, KeyEntrySchema, LookEntrySchema], {
    ...
    error: () => 'each input entry must be one of: {action_name, strength?}, {joy_button, device?}, ' +
...
export function compileInputEntries(inputs) {
    const wire = [];
...
export function entryLabel(e) {
    if ('action_name' in e)
        return e.strength !== undefined ? `${e.action_name}@${e.strength}` : e.action_name;
    if ('joy_button' in e)
        return `joy:${e.joy_button}`;
    if ('axis' in e)
        return `${e.axis}=${e.value}`;
    if ('key' in e)
        return `key:${e.key}`;
    if ('look' in e)
        return `look:${e.look[0]},${e.look[1]}`;
...
export function inputSkewWarning(inputs, inputKinds) {
    const usesLook = inputs.some((e) => 'look' in e);
    if (usesLook && (inputKinds === undefined || !('look' in inputKinds))) {
        return ('WARNING: relative mouse-look entries were IGNORED — the running addon predates mouse-look ' +
    ...
    if (usesKey && (inputKinds === undefined || !('key' in inputKinds))) {
        return ('WARNING: raw key/modifier entries were IGNORED — the running addon predates raw-key ' +
            'injection (#290). Update the godot-mcp addon and restart the Godot editor.');
    }
    const usesController = inputs.some((e) => (!('action_name' in e) && !('key' in e) && !('look' in e)) ||
        ('strength' in e && e.strength !== undefined));
    if (usesController && inputKinds === undefined) {
        return ('WARNING: joypad/axis entries or analog action strength were IGNORED — the running addon ' +
...
const InputSchema = z.discriminatedUnion('action', [
    ...
    z.object({
        ...
        inputs: z
            ...
            .min(1)
            .describe('Array of inputs to execute. Each entry is one of: a named ACTION (action_name, optional ' +
        ...
        screenshot_at_ms: z
            ...
            .max(8)
            .optional()
            .describe('Optional: millisecond offsets (from sequence start) at which to capture a lossless PNG ' +
        ...
        screenshot_max_width: z
            ...
            .max(1920)
            .optional()
            .describe('Max width in px for captured frames (default 640). Resolution is the real cost lever ' +
    ...
    async execute(args, { godot }) {
        switch (args.action) {
            ...
            case 'sequence': {
                ...
                if (budget > cap) {
                    throw new Error(`Input sequence spans ${budget}ms but a single call can cover at most ${cap}ms ` +
                ...
                const result = await godot.sendCommand('execute_input_sequence', {
                ...
                lines.push(`Captured ${ok.length}/${result.captures.length} frame(s) at offsets ` +
            ...
            case 'type_text': {
                ...
                const cap = INPUT_BUDGET_CAP_MS;
                if (budget > cap) {
                    throw new Error(`Typing ${args.text.length} chars at ${args.delay_ms ?? 50}ms each spans ${budget}ms, over the ` +
                        `${cap}ms single-call ceiling. Type in smaller chunks or lower delay_ms.`);
                }
                const t = deriveTimeouts(budget, { readyWait: true });
                const result = await godot.sendCommand('type_text', { text: args.text, delay_ms: args.delay_ms, submit: args.submit, relay_timeout_ms: t.relayMs }, { timeoutMs: t.serverMs });
                if (result.error) {
                    throw new Error(result.error);
                }
