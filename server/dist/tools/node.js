...
const NodeReadSchema = z
    ...
    z.object({
        action: z
            .literal('find')
            .describe('Find nodes by name and/or type. Searches the RUNNING game\'s live tree when a game is playing (spawned entities included); otherwise searches the scene open in the editor.'),
        name_pattern: z
            .string()
            .optional()
    ...
    .refine((data) => (data.action === 'find' ? !!data.name_pattern || !!data.type : true), { message: 'find requires name_pattern and/or type' });
    ...
    async execute(args, { godot }) {
        switch (args.action) {
            ...
            case 'get_scene_tree': {
                ...
                return structured(result.tree);
            }
            case 'find': {
                const result = await godot.sendCommand('find_nodes', {
...
export const nodeEdit = defineTool({
    ...
    description: 'Modify scene nodes in the editor: update a node\'s properties, or reparent it (the editor rewrites child paths and signal connections correctly; hand-editing .tscn for a reparent does not). Use it to change existing nodes in the open scene. To inspect properties, the scene tree, or search for nodes, use godot_node_read; to add or remove nodes, or attach scripts and connect signals, edit the .tscn file directly, then verify with godot_node_read\'s get_scene_tree.',
    ...
    async execute(args, { godot }) {
        switch (args.action) {
            ...
            case 'reparent': {
                const result = await godot.sendCommand('reparent_node', {
                    node_path: args.node_path,
...
export const nodeTools = [nodeRead, nodeEdit];
