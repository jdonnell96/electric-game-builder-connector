...
export function summarizeNumericField(samples, windowMs) {
    if (samples.length === 0) {
...
export function summarizeStringField(samples) {
    if (samples.length === 0) {
...
function summarizeWatchRaw(raw) {
    const result = {};
...
export function buildTimeline(events, fields) {
    const timeline = [];
    ...
    for (const [key, summary] of Object.entries(fields)) {
        if (!('changes' in summary))
            continue; // numeric events stay per-field only
...
function summarizeWatchResponse(raw) {
    ...
    const stringFieldTruncated = Object.values(fields).some((f) => 'changes' in f && f.samples_truncated === true);
...
// ── Schema ───────────────────────────────────────────────────────────────────
const INCLUDE_VALUES = ['transform', 'velocity', 'anim', 'groups', 'onscreen', 'state', 'ui'];
const RuntimeStateSchema = z.discriminatedUnion('action', [
    z.object({
        action: z
            .literal('digest')
            .describe('Snapshot current game entity state as structured JSON — exact positions, velocities, ' +
        ...
        select: z
            ...
            .optional()
            .describe('Selection tier: "group" = nodes in mcp_watch group, "method" = nodes with _mcp_state(), ' +
        ...
        paths: z
            .array(z.string())
            .optional()
            .describe('Explicit absolute node paths to include in addition to tier selection, e.g. ' +
    ...
    z.object({
        action: z
            .literal('watch_start')
            .describe('Start an in-engine sampler that records specified node fields over a time window. ' +
        ...
        signals: z
            ...
            .max(16)
            .optional()
            .describe('Signals to record as discrete timeline events during the window. Each emission is ' +
    ...
    z.object({
        action: z
            ...
            .describe('Collect the current sampler buffer and return a per-field summary ' +
    ...
    z.object({
        action: z
            .literal('watch_stop')
            .describe('Stop the sampler early (disconnecting watched signals) and return the final ' +
...
export const runtimeState = defineTool({
    ...
    description: 'Observe live game state as structured data. ' +
    ...
    async execute(args, { godot }) {
        switch (args.action) {
            case 'digest': {
                ...
                const result = await godot.sendCommand('get_runtime_state', params);
                if (result.nodes_truncated) {
                    const note = `TRUNCATED: ${result.nodes_returned} of ${result.nodes_total_matched} matching nodes returned ` +
            ...
            case 'watch_start': {
                const watchResult = await godot.sendCommand('watch_start', {
                    specs: args.specs ?? [],
                    hz: args.hz ?? 20,
                ...
                if (unresolvedFields.length > 0) {
                    warnings.push(`unresolved fields: ${unresolvedFields.map((u) => `${u.path}:${u.field} (${u.reason})`).join(', ')}. ` +
                ...
                if (requestedSignals > 0) {
                    ...
                    if (watchResult.connected_signals === undefined) {
                        warnings.push('signals were IGNORED: the running addon predates the signal timeline feature — update the godot-mcp addon.');
                    }
                    else if (watchResult.connected_signals + unresolved.length < requestedSignals) {
                        warnings.push(`${requestedSignals - watchResult.connected_signals - unresolved.length} signal spec(s) were dropped in ` +
            ...
            case 'watch_collect': {
                return summarizeWatchResponse(await godot.sendCommand('watch_collect'));
            }
            case 'watch_stop': {
                return summarizeWatchResponse(await godot.sendCommand('watch_stop'));
            }
        }
    },
});
export const runtimeStateTools = [runtimeState];
