...
function stepBudgetMs(args) {
    if (args.action === 'step') {
        if (args.duration_ms !== undefined)
            return args.duration_ms;
...
export const gameTime = defineTool({
    ...
    async execute(args, { godot }) {
        switch (args.action) {
            case 'freeze': {
                const result = await godot.sendCommand('game_time_freeze');
            ...
            case 'step': {
                ...
                const result = await godot.sendCommand('game_time_step', {
            ...
            case 'step_until': {
                const t = deriveTimeouts(stepBudgetMs(args));
                const result = await godot.sendCommand('game_time_step_until', {
                    until: args.until,
            ...
            case 'thaw': {
                const result = await godot.sendCommand('game_time_thaw');
            ...
            case 'status': {
                const result = await godot.sendCommand('game_time_status');
                return structured(result);
            }
        }
    },
});
export const gameTimeTools = [gameTime];
//# sourceMappingURL=game-time.js.map