...
export const tilemapEdit = defineTool({
    ...
    description: 'Modify TileMapLayer cells in the open scene: set a single cell, erase a cell, clear a whole layer, or set many cells in one batch. Use it to paint or remove tiles; cell data is stored base64-encoded in the .tscn, so editing the file is not an alternative, and set_cells_batch beats repeated set_cell calls for anything beyond a few tiles. To inspect layers, TileSets, or existing cells without changing anything, use godot_tilemap_read.',
    ...
    async execute(args, { godot }) {
        switch (args.action) {
            ...
            case 'erase_cell': {
                const result = await godot.sendCommand('erase_cell', {
                    node_path: args.node_path,
                    coords: args.coords,
                });
            ...
            case 'clear_layer': {
                const result = await godot.sendCommand('clear_layer', {
                    node_path: args.node_path,
                });
                return `Cleared layer: ${result.cells_removed} cells removed`;
            }
            case 'set_cells_batch': {
                const result = await godot.sendCommand('set_cells_batch', {
                    node_path: args.node_path,
                    cells: args.cells,
                });
...
export const gridmapEdit = defineTool({
    ...
    description: 'Modify GridMap cells in the open scene: set a single cell to a MeshLibrary item with an orientation, clear a cell, clear the whole map, or set many cells in one batch. Use it to build or remove 3D grid content; cell data is stored base64-encoded in the .tscn, so editing the file is not an alternative, and set_cells_batch beats repeated set_cell calls for anything beyond a few cells. To inspect the map, its MeshLibrary, or existing cells without changing anything, use godot_gridmap_read.',
    schema: GridmapEditSchema,
    async execute(args, { godot }) {
        switch (args.action) {
            case 'set_cell': {
                const result = await godot.sendCommand('set_gridmap_cell', {
                    node_path: args.node_path,
                    coords: args.coords,
                    item: args.item,
            ...
            case 'clear_cell': {
                const result = await godot.sendCommand('clear_gridmap_cell', { node_path: args.node_path, coords: args.coords });
                return `Cleared cell at (${result.cleared.x}, ${result.cleared.y}, ${result.cleared.z})`;
            }
            case 'clear': {
                const result = await godot.sendCommand('clear_gridmap', {
                    node_path: args.node_path,
                });
                return `Cleared GridMap: ${result.cells_removed} cells removed`;
            }
            case 'set_cells_batch': {
                const result = await godot.sendCommand('set_gridmap_cells_batch', {
                    node_path: args.node_path,
                    cells: args.cells,
                });
                return `Set ${result.cells_set} cells`;
...
export const tilemapTools = [tilemapRead, tilemapEdit, gridmapRead, gridmapEdit];
