...
const ProjectSchema = z.discriminatedUnion('action', [
    ...
    z.object({
        action: z
            .literal('check_stale')
            .describe('Check whether project.godot was edited on disk after the editor loaded it, leaving the ' +
...
export const project = defineTool({
    ...
    async execute(args, { godot }) {
        switch (args.action) {
            ...
            case 'check_stale': {
                const result = await godot.sendCommand('get_project_staleness');
...
export const projectTools = [project];
