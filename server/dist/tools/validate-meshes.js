...
export const validateMeshes = defineTool({
    ...
    description: 'Check every code-built (ArrayMesh) surface in the RUNNING game for silent data corruption ' +
    ...
    async execute(args, { godot }) {
        const result = await godot.sendCommand('validate_meshes', {
        ...
        if (result.checked_surfaces === 0) {
            ...
            return (`No mesh data was validated (${reason}). ` +
        ...
        if (result.total_findings === 0) {
            return (`Checked ${result.checked_meshes} meshes (${result.checked_surfaces} surfaces) — no integrity problems. ` +
...
export const validateMeshesTools = [validateMeshes];
//# sourceMappingURL=validate-meshes.js.map