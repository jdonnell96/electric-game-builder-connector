...
export function exampleForProp(name, prop, toolName) {
    ...
    if (toolOverrides && name in toolOverrides)
        return toolOverrides[name];
    ...
    switch (prop.type) {
        ...
        case 'object': {
            ...
            for (const key of Object.keys(props)) {
                if (req.includes(key))
                    obj[key] = exampleForProp(key, props[key], toolName);
            }
            return obj;
        }
        default:
...
export function buildVariantExample(variant, toolSchema, toolName) {
