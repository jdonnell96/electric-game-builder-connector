...
vi.mock('../../testing/run-tests.js', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../../testing/run-tests.js')>();
  return { ...actual, runTests: (...args: unknown[]) => runTests(...args) };
});
...
describe('godot_run_tests tool', () => {
  ...
  it('uses the connected project and returns text plus structured content', async () => {
    ...
    const r = result as { text: string; structuredContent: unknown };
