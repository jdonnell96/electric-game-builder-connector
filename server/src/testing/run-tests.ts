...
import { parseGodotLog, type GodotScriptError } from '../installer/godot-log.js';

// Run a project's test scene in a headless Godot and turn what it printed into
// a structured verdict. The protocol is the one McDaniels' smoke test already
// speaks and the starter kits inherit:
//
//   PASS  <label>
//   FAIL  <label>  <detail>
//   <anything>: <n> checks, <m> failures      (optional summary line)
//
// and a non-zero exit code on failure. A `[tag]` prefix before PASS/FAIL is
// tolerated so harnesses can namespace their output. Godot's own SCRIPT ERROR
// lines are collected too, because a test that crashes prints no FAIL at all.
...
export function parseTestLog(text: string): ParsedTestLog {
  ...
  for (const raw of text.split(/\r?\n/)) {
    ...
    if (SUMMARY_RE.test(line)) summary = line.trim();
  }
...
export function formatTestRunResult(result: TestRunResult): string {
  ...
  for (const c of result.checks.filter((c) => !c.ok)) lines.push(`  FAIL  ${c.label}${c.detail ? `  ${c.detail}` : ''}`);
  for (const e of result.errors.slice(0, 10)) lines.push(`  ERROR ${e.message}${e.file ? ` (${e.file}${e.line !== undefined ? `:${e.line}` : ''})` : ''}`);
  if (result.summary) lines.push(`  ${result.summary}`);
  if (!result.ok && result.log_tail.length > 0) {
    lines.push('  Output tail:');
    for (const l of result.log_tail.slice(-12)) lines.push(`    ${l}`);
  }
  if (result.log_path) lines.push(`  Full output: ${result.log_path}`);
  return lines.join('\n');
}

export function buildGodotTestArgs(projectPath: string, target: string, userArgs: string[]): string[] {
  const base = ['--headless', '--path', projectPath];
  if (/\.gd$/i.test(target)) base.push('--script', target);
  else base.push(target);
  if (userArgs.length > 0) base.push('--', ...userArgs);
  return base;
}
...
export async function runTests(opts: RunTestsOptions): Promise<TestRunResult> {
  ...
  const candidates =
    opts.candidates ?? (opts.godot ? findGodotCandidates(opts.godot).filter((c) => c.source === '--godot') : findGodotCandidates());
  ...
  const run = await new Promise<{ text: string; exitCode: number | null; timedOut: boolean }>((resolveRun) => {
    const chunks: string[] = [];
