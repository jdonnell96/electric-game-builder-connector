...
import { DELETE_LEFTOVER_RE, MANIFEST_FILE, hashFile, readManifest, verifyAddonDir, type AddonManifest } from './manifest.js';
...
export function defaultBundledAddonDir(): string {
...
export async function installAddon(projectPath: string, options: InstallOptions = {}): Promise<InstallResult> {
  ...
  if (bundle.recovered.length > 0) result.recovered = bundle.recovered;
...
interface BundleSources {
  /** Manifest path -> file to copy from. */
  sources: Map<string, string>;
  recovered: string[];
  damaged: string[];
}

function resolveBundleSources(bundledAddon: string, manifest: AddonManifest): BundleSources {
  const out: BundleSources = { sources: new Map(), recovered: [], damaged: [] };
...
function findLeftoverWithHash(source: string, sha: string): string | undefined {
  ...
  for (const name of readdirSync(dir)) {
    ...
    if (safeHash(path) === sha) return path;
  }
  return undefined;
}
...
export function findNpxCacheEntry(path: string): string | undefined {
...
export function describeDamagedBundle(bundledAddon: string, damaged: string[]): string {
  const lines = [
