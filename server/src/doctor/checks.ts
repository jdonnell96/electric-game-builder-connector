...
import { runAddonCompileCheck, type WorkingGodot } from '../installer/compile-check.js';
...
import { ancestors, describeProcess, findProcess, peerOf, type ProcessInfo, type SystemSnapshot } from './system.js';

// Every check answers one question and names the fix. They take the system
// snapshot and paths as arguments rather than looking things up themselves,
// so the whole diagnosis of a machine can be replayed in a unit test.
...
export function isGodotProcess(p: ProcessInfo): boolean {
...
function executableOf(commandLine: string): string {
...
export function isGodotGame(p: ProcessInfo): boolean {
  return isGodotProcess(p) && /--remote-debug\b|--editor-pid\b/.test(p.commandLine);
}

export function isGodotEditor(p: ProcessInfo): boolean {
  return isGodotProcess(p) && !isGodotGame(p) && !/--headless\b/.test(p.commandLine);
}

export function projectPathFromCommandLine(commandLine: string): string | undefined {
...
export function isOtherGodotTool(p: ProcessInfo): boolean {
  if (!/node|bun|deno|python|dotnet/i.test(p.name)) return false;
  if (!/godot/i.test(p.commandLine)) return false;
  if (isOurServer(p)) return false;
  if (/npx-cli\.js|npm-cli\.js/i.test(p.commandLine)) return false;
  return true;
}

function stopCommand(pid: number, platform: NodeJS.Platform): string {
  return platform === 'win32' ? `Stop-Process -Id ${pid}` : `kill ${pid}`;
}

function parentNote(snapshot: SystemSnapshot, pid: number): string {
...
export function checkNodeVersion(version: string = process.version): Finding {
  const major = Number(version.replace(/^v/, '').split('.')[0]);
  if (major >= 20) return { id: 'node', status: 'ok', title: `Node ${version}` };
  return {
    id: 'node',
    status: 'fail',
...
export function checkServerBuild(serverVersion: string): Finding {
...
export function checkGodotBinary(godot: WorkingGodot | undefined, explicit?: string): Finding {
  if (godot) {
...
export function checkEditors(snapshot: SystemSnapshot): EditorSummary {
...
export function checkDebugPort(snapshot: SystemSnapshot, port: number = DEBUG_PORT): DebugPortSummary {
  if (snapshot.errors.length > 0) {
  ...
  if (foreign.length > 0) {
    ...
    detail.push(
      `Port ${port} is Godot's remote debugger. The editor listens and the running game is meant to be its only peer; any other connection corrupts the channel, and every exec, screenshot, game_time and runtime_state call then times out with no error in the editor.`,
    );
    return {
      finding: {
        id: 'port_6007',
...
export function checkBridgePort(snapshot: SystemSnapshot, serverVersion: string, port: number = DEFAULT_BRIDGE_PORT): BridgePortSummary {
  if (snapshot.errors.length > 0) {
  ...
  const clients: ProcessInfo[] = [];
  const clientPids = new Set<number>();
  for (const c of snapshot.connections) {
    if (c.state !== 'established' || c.localPort !== port) continue;
    const peer = peerOf(snapshot, c);
  ...
  const extra: Finding[] = [];
  for (const client of clients) extra.push(...inspectConnectedServer(client, serverVersion, snapshot));
...
function inspectConnectedServer(client: ProcessInfo, serverVersion: string, snapshot: SystemSnapshot): Finding[] {
  if (!isOurServer(client)) return [];
...
function npxEntryOf(dir: string): string | undefined {
  const parts = dir.split(/[\\/]/);
  const i = parts.lastIndexOf('_npx');
  return i >= 0 && i + 1 < parts.length ? parts.slice(0, i + 2).join(dir.includes('\\') ? '\\' : '/') : undefined;
}

export function checkServerProcesses(snapshot: SystemSnapshot, holderPid: number | null, excludePids: number[] = []): Finding[] {
...
export function checkProject(projectPath: string, serverVersion: string, bundledAddonDir: string = defaultBundledAddonDir()): ProjectSummary {
  const findings: Finding[] = [];
...
export async function checkAddonCompile(addonDir: string, godot: WorkingGodot): Promise<Finding> {
...
const TEMPLATE_PLATFORMS: Array<{ name: string; file: RegExp }> = [
...
export function templateVersionDir(fullVersion: string): string | undefined {
  return fullVersion.match(/^(\d+\.\d+(?:\.\d+)?\.(?:stable|rc\d*|beta\d*|alpha\d*|dev\d*))/)?.[1];
}

export function exportTemplatesRoot(platform: NodeJS.Platform, env: NodeJS.ProcessEnv = process.env, home: string = homedir()): string {
  if (platform === 'win32') return join(env.APPDATA ?? join(home, 'AppData', 'Roaming'), 'Godot', 'export_templates');
...
export function checkExportTemplates(
  fullVersion: string,
